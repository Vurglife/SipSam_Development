...
function createCardEl(cardCode, draggable=false) {
    ...
    el.textContent  = getDisplayValue(cardCode) + getSuitSymbol(cardCode);
    if (draggable) {
        el.draggable = true;
    ...
    [['1',3],['2',5],['3',5]].forEach(([num, count]) => {
        ...
        if (revealed && cards.length > 0) {
            cards.forEach(c => {
                ...
                m.textContent = getDisplayValue(c) + getSuitSymbol(c);
                slot.appendChild(m);
            });
    ...
    [['hand1', 3], ['hand2', 5], ['hand3', 5]].forEach(([key, count]) => {
        ...
        if (cards.length > 0) {
            cards.forEach(c => {
                ...
                el.dataset.card = c;
                el.textContent  = getDisplayValue(c) + getSuitSymbol(c);
                zone.appendChild(el);
            });
        }
...
function updateOpponentSeats(state, revealed) {
    ...
    // ── All non-banker players → p1, p2, p3 (including me if not banker) ──
    const others = Object.entries(players).filter(([sid]) => sid !== bankerSid);
    others.forEach(([sid, player], i) => {
        const zId = zones[i]; if (!zId) return;
        ...
        // Always render on table — face-up when revealed, face-down while arranging
        renderOpponentHands(zId, player, revealed);
    });

    // Hide unused zones
    for (let i = others.length; i < 3; i++) {
        const nameEl = document.getElementById(`${zones[i]}-name`);
