...
- **Rhum32 public Multiplayer quick-join + special-win announcements.** Public Multiplayer room selection now mirrors SipSam's strict matching by table tier and selected round count, but uses Rhum32's no-bot rule: it matches only active rooms with a real open seat and at least one future round remaining; waiting rooms, full rooms, wrong-round rooms, and final-round rooms create a fresh host room instead. Late joiners who enter after a hand has started are seated but held out until the next round, so they cannot bet or decide without cards. Rhum32 settlement now publishes public special-win fields (`player`, `specialName`, `bonus`, `totalWin`) for every positive-net special, the table announcement shows Player / Special / Bonus / Total Win, and special wins trigger a larger chip-flow cascade toward the winning seat. Cache-bust `style.css?v=21`, `game.js?v=29`.

- **SipSam side-bet prompt delivery + next-round timing fix.** Side-bet challenges now send a direct targeted `sideBetOffer` message to eligible real players, so Beat Hand targets and First Special / Best Card participants receive an on-screen Accept/Decline/Ignore modal instead of relying only on passive state redraw. Beat Hand and Best Card accepted pots now lock for the next round and do not resolve against the already-finished reveal round. Best Card now normalizes deck `T` as UI `10`, carries when no participant holds the selected value, tops up current participants on later rounds, and refunds unwon pots at game end. Cache-bust `style.css?v=36`, `game.js?v=48`.

- **SipSam side-bet panel overlap fix.** Side Bets panel now reserves horizontal space for the fixed countdown timer on desktop, moves below the timer on narrower tablet widths, and the side-bet offer modal uses a higher z-index so Accept/Decline/Ignore prompts cannot hide behind the countdown or menu. Cache-bust `style.css?v=37`, `game.js?v=49`.

- **SipSam side-bet WebSocket command envelope fix.** The client no longer sends the selected side-bet kind as `data.type`, because `sendMsg()` uses `type` as the WebSocket action and was overwriting `initiateSideBet` / `acceptSideBet` / `declineSideBet` with `beatHand`, `bestCard`, or `firstSpecial`. Side-bet payloads now use `sideBetType`, and the server accepts `sideBetType` while keeping backward compatibility. Cache-bust `style.css?v=38`, `game.js?v=50`.

- **SipSam side-bet live prompt validation.** After restarting SipSam on ports `2999`/`3001`, a live three-human WebSocket smoke test reached reveal, sent Beat Hand from one non-banker to another, confirmed the target received the `sideBetOffer`, accepted it, and confirmed the pot locked. Product-owner browser tabs still need hard refresh before retesting.

- **Platform game launch routes restored.** The platform server now serves the installed game clients at `/roulette`, `/rhum32`, and `/blackjack` in addition to `/sipsam`, so dashboard game cards no longer fall through to `Cannot GET`. Matchmaking calls through `localhost:3000/matchmake/...` are now forwarded explicitly to the correct game server: Roulette `3005`, Rhum32 `2998`, SipSam `2999`.

- **SipSam side-bet prompt and settlement repair.** Side-bet offers now surface immediately during reveal as an explicit in-game prompt with Accept, Decline, and Ignore while keeping the existing side-bet panel; responders no longer have to wait for the later 7s `sideBetPhase`. `sideBetPhase` is treated as a mid-round exit phase. Beat Hand and Best Card pots accepted during the response window now resolve after that window, before round end. Side-bet wallet movements are logged through `/api/game/side-bet-transaction`. Cache-bust `style.css?v=32`, `game.js?v=47`.
