...
- **SipSam side-bet prompt and settlement repair.** Side-bet offers now surface as an explicit in-game prompt with Accept, Decline, and Ignore while keeping the existing side-bet panel. `sideBetPhase` is treated as a mid-round exit phase. Beat Hand and Best Card pots accepted during the 7s response window now resolve after that window, before round end. Side-bet wallet movements are logged through `/api/game/side-bet-transaction`. Cache-bust `style.css?v=31`, `game.js?v=46`.
